# HTML y CSS

**HTML** es un lenguaje que nos permite definir **la estructura** de las páginas Web. **CSS** es un lenguaje que nos permite definir **el formato \(los estilos\)** de los elementos HTML.

## Prerequisitos

* [Preparación](../preparacion/)

## Contenido

* [Introducción a HTML](introduccion-a-html.md)
* [Introducción a CSS](introduccion-a-css.md)
* [Más elementos de HTML](mas-elementos-de-html.md)
* [Tablas](tablas.md)
* [Formularios](formularios.md)
* [El modelo de caja en CSS](el-modelo-de-caja-en-css.md)
* [Fondos \(backgrounds\)](fondos-backgrounds.md)
* [Posicionamiento](posicionamiento.md)
* [Selectores CSS](selectores-css.md)
* [Bordes, sombras y gradientes](bordes-sombras-y-gradientes.md)
* [Media Queries](media-queries.md)
* [Unidades en CSS](unidades.md)
* [Flexbox](flexbox.md)
